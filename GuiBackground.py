...
def createEnsemDendrogram(data,metab_data,norm=1,link='ward',dist='euclidean',func="ensemble"):
    ...
    metaboliteDend = dendrogram(linkageMetabOut, orientation='left')
...
def create_dendrogram(data, norm=1,link='ward',dist='euclidean',func='clustergram'):
    ...
    This function outputs a heatmap and sends a call to the plotting function to create the clustergram. 
    '''
...
def recClusters(dataFinal,heatmapAxes,groupDendLeaves,metab_data):
    ...
    while j < ensemMetabs:
        ...
        if len(found[0])==1:
            ...
            plt.text(j,j,"1",color='red',fontsize=7)
            j += 1
            del(arrays)
        else:
            ...
            plt.text(midPoint,midPoint,numMetabs,color='red',fontsize=7)
            j = maxMetab + 1
            del(arrays)
    return
